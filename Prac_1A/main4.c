...
void handle_buttons(void)
{
    ...
    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET) {
        if (now - last_button_time[0] >= DEBOUNCE_MS) {
...
void set_mode(LED_Mode new_mode)
{
    ...
    sparkle_pattern = 0;
    sparkle_display_until = 0;
    sparkle_next_off_time = 0;
    sparkle_off_index = 0;

    for (uint8_t led = 0; led < 8; led++) {
    	sparkle_leds_on[led] = 0;
    }
...
void mode1_update(void)
{
	...
	// Update the current_led index based on direction
	current_led += direction;

	// Handle direction reversal at the edges (without duplicating states)
	if (current_led == 0) {
		direction = 1;
	}
...
void mode2_update(void)
{
	...
	// Iterate through and turn on all the leds
	for (uint8_t i=0; i<8; i++) {
	    turn_on_led(i);
	}
    ...
    switch (sparkle_state) {
        case SPARKLE_IDLE:
        	...
        	for (int led=0; led<8; led++) {
        		if ((sparkle_pattern >> led) & 1) {
        			turn_on_led(led);
        			sparkle_leds_on[led] = 1;
        		}else {
        			turn_off_led(led);
        			sparkle_leds_on[led] = 0;
        		}
        	...
        	sparkle_display_until = now + 100 + (rand() % (1500 - 100 + 1));
        ...
        case SPARKLE_DISPLAY:
        	...
        	if (now >= sparkle_display_until) {
        		sparkle_display_until = now + 100 + (rand() % (150 - 100 + 1));
        ...
        case SPARKLE_TURN_OFF:
        	...
        	if (now >= sparkle_display_until) {
        		...
        		// Check if any LEDs are still on
        		for (int led=0; led < 8; led++) {
        			if (sparkle_leds_on[led]) {
        				leds_still_on = 1;
        				break;
